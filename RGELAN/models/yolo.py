...
from models.common import AConv, Conv, ConvTranspose, Bottleneck, SPP, SPPF, DWConv, BottleneckCSP, DWConvTranspose2d, SPPCSPC, ADown, RepNCSPELAN4, SPPELAN, Bottleneck, BottleneckCSPA, BottleneckCSPB, BottleneckCSPC, RepBottleneck, RepBottleneckCSPA, RepBottleneckCSPB, RepBottleneckCSPC, Res, ResCSPA, ResCSPB, ResCSPC, RepRes, RepResCSPA, RepResCSPB, RepResCSPC, ResX, ResXCSPA, ResXCSPB, ResXCSPC, RepResX, RepResXCSPA, RepResXCSPB, RepResXCSPC, Depth, Concat, Shortcut, ReOrg, CBLinear, CBFuse, Contract, Expand, DFL
import contextlib
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    anchors = d.get('anchors', 3)  # default to 3 anchors if not specified
    nc = d.get('nc', 80)  # default to 80 classes if not specified
    gd = d.get('depth_multiple', 1.0)  # default depth multiple
    gw = d.get('width_multiple', 1.0)  # default width multiple
    act = d.get('activation', None)  # default activation
    ...
    for i, (f, n, m, args) in enumerate(d.get('backbone', []) + d.get('head', [])):  # from, number, module, args
        ...
        if m in {
            ...
            RepNCSPELAN4, SPPELAN, nn.Conv2d, RepVGG, Bottleneck, BottleneckCSPA, BottleneckCSPB, BottleneckCSPC,
            RepBottleneck, RepBottleneckCSPA, RepBottleneckCSPB, RepBottleneckCSPC, Res, ResCSPA, ResCSPB, ResCSPC, 
            RepRes, RepResCSPA, RepResCSPB, RepResCSPC, ResX, ResXCSPA, ResXCSPB, ResXCSPC, RepResX, RepResXCSPA, RepResXCSPB, 
            RepResXCSPC, Depth}:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, SPPCSPC, BottleneckCSPA, BottleneckCSPB, BottleneckCSPC,
