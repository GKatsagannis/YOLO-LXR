...
from models.squeezenet import Fire  # Import Fire module
...
class DAFM(nn.Module):
    def __init__(self, channels):
        ...
        self.deform_conv = nn.Conv2d(channels, channels, kernel_size=3, padding=1, stride=1, bias=False, groups=channels)
        self.bn = nn.BatchNorm2d(channels)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        x = self.conv1(x)
        x = self.deform_conv(x)  # Deformable conv layer
        x = self.bn(x)
        x = self.relu(x)
        return x
...
class DAFM2(nn.Module):
    def __init__(self, channels, *args, **kwargs):
        super(DAFM2, self).__init__()
        self.channels = channels

        # Offset Network (A simple one for demonstration)
        self.offset_conv = nn.Conv2d(channels, channels, kernel_size=3, padding=1, stride=1, bias=False, groups=channels)
        self.offset_bn = nn.BatchNorm2d(channels)
        self.offset_relu = nn.ReLU(inplace=True)

        # Attention mechanism (simplified version)
        self.query_conv = nn.Conv2d(channels, channels // 8, kernel_size=1)  # Reduced query size
        self.key_conv = nn.Conv2d(channels, channels // 8, kernel_size=1)  # Reduced key size
    ...
    def forward(self, x):
        ...
        offset = self.offset_relu(offset)

        # Attention mechanism part
        query = self.query_conv(x)  # Reduced query size
        key = self.key_conv(x)      # Reduced key size
        value = self.value_conv(x)

        # Flattening for matrix multiplication
        batch_size, channels, height, width = query.size()
        
        # Reshaping query and key for matrix multiplication
        query = query.view(batch_size, channels // 8, -1).permute(0, 2, 1)  # [batch, height*width, channels//8]
        key = key.view(batch_size, channels // 8, -1)  # [batch, channels//8, height*width]
        
        attention = torch.matmul(query, key)  # Matrix multiplication
        attention = F.softmax(attention, dim=-1)

        # Compute output
        value = value.view(batch_size, channels, -1)  # [batch, channels, height*width]
        out = torch.matmul(attention, value.permute(0, 2, 1))  # Matrix multiplication
        ...
        return x + out  # Add the attention output to the result
